...
black_sleeve.loc_vars = function(self)
	local key, vars
	if PSI.get_gameset().unfiltered and (self.get_current_deck_key() == "b_para_voiddeck" or self.get_current_deck_key() == "b_para_chasmdeck") then
        key = self.key .. "_alt"
...
painted_sleeve.loc_vars = function(self)
	local key, vars
	if PSI.get_gameset().unfiltered and (self.get_current_deck_key() == "b_para_artisticdeck" or self.get_current_deck_key() == "b_para_chasmdeck") then
        key = self.key .. "_alt"
